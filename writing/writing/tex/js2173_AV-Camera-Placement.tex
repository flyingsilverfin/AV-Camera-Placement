...
the geometric modeling performed in following chapters. However, 
my implementation is also capable of handling non-zero roll.
...
There are a variety of ways of representing curves that have been
used in the literature.

One simple option is to discretize any curve as a set of linear
piecewise segments~\cite{bodor2007optimal}, where it was used to 
...
Euler curves, which allow linearly varying curvature,
but these were deemed overly complex for the purpose of this work.

During implementation I began representation of rotations as
quaternions and translations as vectors, but this was very error prone. 
In the end I used homogenous coordinates to joinly store translation
...
post-calibration mean, max finishing deviation: (2.0613898119306162, 5.8511940475902247)
post calibration mean, max finishing angular deviation: 0.257, 1.014 (degrees)
...
correction based on the vehicle's instantaneous velocity. To further
avoid partially stale updates from corrupting the vehicle's state,
I also made the oval 50\% more circular, as defined by the ratio
of its major to minor axis lengths. These worked very well in practice.

The issue of timing delays is not considered further, and could
be dealt with similarly in a physical implementation.
...
* mention time correction for camera update!
* in practice time disturbances in the simulation resulted in ~100ms delays which could only partially be compensated for
=> induce strange behavior in EKF since one coordinate is very unlikely though this is just due to time delay
=> make oval more circular by 50\% works well in practice 
