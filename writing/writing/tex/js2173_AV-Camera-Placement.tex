...
Unfortunately, using the Kalman-gain based objective function
...
could only be sent a single update, how can the camera best
be oriented to reduce its localization error, on average.
...
can only shrink as a result of a measurement, so there
must some utility in taking advantage of sending
multiple measurements to the vehicle.
