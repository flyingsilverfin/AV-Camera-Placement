...
void PriusHybridPlugin::Load(physics::ModelPtr _model, sdf::ElementPtr _sdf)
{
  ...
  if (!dPtr->chassisLink)
  {
    std::cerr << "ERR: could not find chassis link" << std::endl;
    return;
  }
...
*/
  ...
  std::cout << "INFO subscribing to /keypress" << std::endl;
...
//  this->dataPtr->evMode = !this->dataPtr->evMode;
    ...
    std::cerr << "CmdMode msg received, but this is now disabled" << std::endl; 
...
std::cerr << "Received keypress!" << std::endl;
