...
if __name__ == "__main__":
    ...
    compute_MI_string = '--compute_MI' if args.compute_MI else '--no-compute_MI' 
    ...
    for i in range(n-1):
        ...
        s = 'export ROS_MASTER_URI=http://localhost:{0}/; export GAZEBO_MASTER_URI=${{GAZEBO_MASTER_URI:-"http://localhost:{6}"}}; python -m {4}.start_single --port={0} --repeats={1} --config={2} --out={3} {5} {7} {8}'.format(ros_port, runs_per_instance, config, out_dir, __package__, rviz_string, gazebo_port, continue_string, compute_MI_string)
    ...
    s = 'export ROS_MASTER_URI=http://localhost:{0}/; export GAZEBO_MASTER_URI=${{GAZEBO_MASTER_URI:-"http://localhost:{6}"}}; python -m {4}.start_single --port={0} --repeats={1} --config={2} --out={3} {5} {7} {8}'.format(ros_port, last_instance_runs, config, out_dir, __package__, rviz_string, gazebo_port, continue_string, compute_MI_string)
    print("Launching with: " + s)
    proc = subprocess.Popen([s], shell=True)
    runners.append(proc)
    ...
    if args.compute_MI:
        aggregate_metrics["mean mutual inf"] = {
            "mutual inf": 0,
            "variance": 0,
            "stddev": 0,
            "num_bags": 0
