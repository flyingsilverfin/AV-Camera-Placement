...
def camera_grid_eval(name, camera_description, vehicle_grid_params, save_dir, model_types=['perspective', 'stereographic', 'equidistance']):
    # instantiate camera
    ...
    for (counter, vehicle_center, vehicle_orientation) in camera_grid_generator(cameras[0], vehicle_grid_params):
        for size in vehicle_sizes:
            ...
            for camera in cameras:
                ...
                target_dir = os.path.join(save_dir, 'gen', 'grid', camera_config_name, description["vehicle_size_string"], "{0}_heading_{1}".format(counter, vehicle_orientation))
...
def process_config(config, save_root_dir):
    ...
    for desc in camera_descriptions:
        if eval_type == 'camera_grid':
            ...
            camera_grid_eval(eval_type, desc, vehicle_grid_params, save_root_dir)
